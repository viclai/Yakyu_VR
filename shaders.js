 ...
 * Fill in this file with subclasses of Shader, each of which will store and
 * manage a complete GPU program.

 * The "vertex_glsl_code" string is loaded by our javascript and then used as
 * the Vertex Shader program.  Our javascript sends this code to the graphics
 * card at runtime, where on each run it gets compiled and linked there. 
 * Thereafter, all of your calls to draw shapes will launch the vertex shader
 * program once per vertex in the shape (three times per triangle), sending
 * results on to the next phase.  The purpose of this program is to calculate
 * the final resting place of vertices in screen coordinates; each of them
 * starts out in local object coordinates.

 * Likewise, the "fragment_glsl_code" string is loaded by our Javascript and
 * then used as the Fragment Shader program, which gets sent to the graphics
 * card at runtime. The fragment shader runs once all vertices in a triangle / 
 * element finish their vertex shader programs, and thus have finished finding
 * out where they land on the screen. The fragment shader fills in (shades)
 * every pixel (fragment) overlapping where the triangle landed. At each pixel
 * it interpolates different values from the three extreme points of the 
 * triangle, and uses them in formulas to determine color. The fragment colors
 * may or may not become final pixel colors; there could already be other 
 * triangles' fragments occupying the same pixels. The Z-Buffer test is applied
 * to see if the new triangle is closer to the camera, and even if so, 
 * blending settings may interpolate some of the old color into the result.
 ****************************************************************************/
          

// ******* THE DEFAULT SHADER: Phong Reflection Model with Gouraud option ****
// (also see Wikipedia)
Declare_Any_Class("Phong_Model",
  {
    ...
    'update_GPU'(g_state, model_transform, material, gpu = this.g_addrs, gl = this.gl) // Define how to synchronize our javascript's variables to the GPU's:
      { 
        ...
        if (g_state.gouraud === undefined) {
          g_state.gouraud = g_state.color_normals = false; // Keep the flags seen by the shader program up-to-date and make sure they are declared.
        }
    ...
    'update_matrices'(g_state, model_transform, gpu, gl) // Helper function for sending matrices to GPU (PCM will mean Projection * Camera * Model)
      {
