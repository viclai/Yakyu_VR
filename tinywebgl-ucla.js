 ...
 * A file to show how to organize a complete graphics program. It wraps common
 * WebGL commands.
 ****************************************************************************/

// Making javascript behave more like Object Oriented C++
function Declare_Any_Class(name, methods, superclass = Object, scope = window) {
  scope[name] = function(args) { // When new gets called on the class name, call its construct():
    if (this.construct)
  ...
  p.define_data_members = function(args) { // Optional utility method for all classes
    for (let i in args)
      this[i] = args[i];
  }
  for (let i in methods) // Include all the other supplied methods.
    p[i] = methods[i];
  scope[name].prototype = p; // The class and its prototype are complete.
}

// *********** SHAPE SUPERCLASS ***********
// Each shape manages lists of its own vertex positions, vertex normals, and 
// texture coordinates per vertex, and can copy them into a buffer in the 
// graphics card's memory. 
// IMPORTANT: When you extend the Shape class, you must supply a populate()
// function that fills in four arrays: One list enumerating all the vertices'
// (vec3) positions, one for their (vec3) normal vectors pointing away from
// the surface, one for their (vec2) texture coordinates (the vertex's 
// position in an image's coordinate space, where the whole picture spans x
// and y in the range 0.0 to 1.0), and usually one for indices, a list of 
// index triples defining which three vertices belong to each triangle. Call
// new on a Shape and add it to the shapes_in_use array; it will populate its
// arrays and the GPU buffers will recieve them.
Declare_Any_Class("Shape",
  {
    ...
    'draw'(graphics_state, model_transform, material, gl = this.gl)
      {
        // The same draw() function is used for every shape -
        // these calls produce different results by varying which
        // vertex list in the GPU we consult.
        if (!this.gl)
          throw "This shape's arrays are not copied over to graphics card yet.";
...
// Colors are just special vec4s expressed as:
// (red, green, blue, opacity) each from 0 to 1.
...
// This class manages a whole graphics program for one on-page canvas,
// including its textures, shapes, shaders, and scenes. In addition to
// requesting a WebGL context, it informs the canvas of which functions to 
// call during events - such as a key getting pressed or it being time to 
// redraw.
Declare_Any_Class("Canvas_Manager",
  {
    'construct'(canvas_id, background_color) 
      {
        var gl, canvas = document.getElementById(canvas_id);
        this.define_data_members({
          shaders_in_use: {},
          shapes_in_use: {},
          textures_in_use: {},
        ...
        for (let name of ["webgl", "experimental-webgl", "webkit-3d", "moz-webgl"])
          if (gl = this.gl = this.canvas.getContext(name)) // Get the GPU ready, creating a new WebGL context for this canvas
            break;
        if (!gl)
    ...
    'register_scene_component'(component)
      {
        ...
        component.init_keys(this.controls);
      },
    'render'(time = 0) // Animate shapes based upon how much measured real time has transpired.
      {                            
        ...
        for (let s in this.shapes_in_use)
          if (!this.shapes_in_use[s].sent_to_GPU)
            this.shapes_in_use[s].copy_onto_graphics_card(this.gl);
...
// Scene_Component Superclass - Base class for any scene part or code snippet we can add to a canvas
Declare_Any_Class("Scene_Component",
  {
    'construct'() {},
    'init_keys'() {},
    'update_strings'() {},
    'display'() {},
    'submit_shapes'(context, shapes) // Store pointers to the shapes locally. Also submit them to the outer context.
    {
      this.shapes = [];
      for (let s in shapes) {
